...
# output
out_dir <- "/data/BB_Bioinformatics/DG/MR_bc_sens/overlapping_ivs/afr"
...
for (protein in proteins) {
  ...
  for (p_thresh in p_thresholds) {
    ...
    all_per_snp[[paste(protein, "AFR", p_thresh, sep = "_")]] <- data.frame(
      Protein       = protein,
      Ancestry      = "AFR",
    ...
    all_summary[[paste(protein, "AFR", p_thresh, sep = "_")]] <- data.frame(
      Protein      = protein,
      Ancestry     = "AFR",
      p_threshold  = p_thresh,
      N_IVs        = nrow(iv_data),
