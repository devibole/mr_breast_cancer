...
if (length(args) == 0) {
  stop("Usage: Rscript run_primary_mr_eas.R <protein_index>")
}
i <- as.numeric(args[1])
...
config <- list(
  ...
  outcome_path = "/data/BB_Bioinformatics/ProjectData/breast_cancer_sum_data/BCAC_BBJ_EAS/EAS_BCAC_BBJ_meta_sumdata.txt.gz",
  ...
  input_export_dir = "/data/BB_Bioinformatics/DG/MR_bc/2025_updated/input_files/eas37",
  results_export_dir = "/data/BB_Bioinformatics/DG/MR_bc/2025_updated/results/eas37",
...
outcome_data[, c("allele1", "allele2") := standardize_alleles(effect_allele_meta, non_effect_allele_meta)]
outcome_data <- outcome_data[outcome_data$chr != "X", ]
...
merged_data <- merged_data %>%
  mutate(
    swap = ALLELE1 != effect_allele_meta,
