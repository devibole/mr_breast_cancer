...
if (length(args) == 0) {
  stop("Usage: Rscript run_primary_mr_afr.R <protein_index>")
}
i <- as.numeric(args[1])
...
config <- list(
  ...
  outcome_path = "/data/BB_Bioinformatics/ProjectData/breast_cancer_sum_data/AABCG_2024_Zheng/AABCG_sum_GRCh37.csv",
  ...
  input_export_dir = "/data/BB_Bioinformatics/DG/MR_bc/2025_updated/input_files/afr37",
  results_export_dir = "/data/BB_Bioinformatics/DG/MR_bc/2025_updated/results/afr37",
...
outcome_data <- data.table::fread(config$outcome_path)
outcome_data <- data.table::setDT(outcome_data)
outcome_data[, c("allele1", "allele2") := standardize_alleles(effect_allele, non_effect_allele)]
outcome_data[, chr := as.numeric(CHR)]
outcome_data[, pos := as.numeric(POS)]
...
merged_data <- merged_data %>%
  mutate(
    swap = ALLELE1 != effect_allele,
    ALLELE1_new = ifelse(swap, ALLELE0, ALLELE1),
    ALLELE0_new = ifelse(swap, ALLELE1, ALLELE0),
    BETA = ifelse(swap, -BETA.x, BETA.x),
    A1FREQ = ifelse(swap, 1 - A1FREQ, A1FREQ)
  ) %>%
  select(-ALLELE1, -ALLELE0) %>%
  rename(ALLELE1 = ALLELE1_new, ALLELE0 = ALLELE0_new)

merged_data$rsids <- merged_data$rsid
merged_data$SE <- merged_data$SE.x
merged_data$beta <- merged_data$BETA.y
merged_data$sebeta <- merged_data$SE.y
